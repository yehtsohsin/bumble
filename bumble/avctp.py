...
# -----------------------------------------------------------------------------
class Protocol:
    Handler = Callable[[int, Optional[bytes]], None]
    command_handlers: Dict[int, Handler]  # Command handlers, by PID
    response_handlers: Dict[int, Handler]  # Response handlers, by PID
    l2cap_channel: l2cap.Channel
    ...
    def on_message(
        ...
        handlers = self.command_handlers if is_command else self.response_handlers
        if pid not in handlers:
            logger.warning(f"no handler for PID {pid}")
            if is_command:
    ...
    def register_handler(
        self, pid: int, handler: Protocol.Handler, registry: Dict[int, Handler]
    ) -> None:
    ...
    def unregister_handler(
        ...
        if pid not in registry or registry[pid] != handler:
            raise ValueError("handler not registered")
        del registry[pid]

    def register_command_handler(self, pid: int, handler: Protocol.Handler) -> None:
        self.register_handler(pid, handler, self.command_handlers)

    def unregister_command_handler(self, pid: int, handler: Protocol.Handler) -> None:
        self.unregister_handler(pid, handler, self.command_handlers)

    def register_response_handler(self, pid: int, handler: Protocol.Handler) -> None:
        self.register_handler(pid, handler, self.response_handlers)

    def unregister_response_handler(self, pid: int, handler: Protocol.Handler) -> None:
        self.unregister_handler(pid, handler, self.response_handlers)
